...
def prune_model(cfg: dict) -> tuple:
    ...
    print(f"Initial Model Size: {model_size / MiB:.2f} MiB, Parameters: {model_parameters}")
    ...
    pruned_cfg = cfg.copy()  # Create a copy to avoid modifying the original config
    ...
    mlflow.register_model(model_uri, f"{cfg['experiment_name']}_pruned_untrained")
